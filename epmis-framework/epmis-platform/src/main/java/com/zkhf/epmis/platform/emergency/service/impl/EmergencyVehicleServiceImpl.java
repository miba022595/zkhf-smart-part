...
import com.zkhf.epmis.platform.mapper.emergency.EmergencyVehicleMapper;
...
public class EmergencyVehicleServiceImpl implements EmergencyVehicleService {
    ...
    @Autowired
    public void setEmergencyVehicleMapper(EmergencyVehicleMapper emergencyVehicleMapper) {
        this.emergencyVehicleMapper = emergencyVehicleMapper;
    }
    ...
    @Override
    public AjaxResult list(EmergencyVehicleReq req) {
        // 请求参数转换
        req = initEmergencyVehicleReq(req);
        if (null == req) {
            return AjaxResult.success();
        }
        boolean page = PageUtils.startPageCheckExists();
        List<EmergencyVehicle> list = emergencyVehicleMapper.selectList(req);
        return PageUtils.getAjaxResult(list, page);
    }

    private EmergencyVehicleReq initEmergencyVehicleReq(EmergencyVehicleReq req) {
        if (null == req) {
            req = new EmergencyVehicleReq();
        }
        req.setVehicleStatusInt(parseVehicleStatus(req.getVehicleStatus()));
        // 添加权限
    ...
    public AjaxResult add(EmergencyVehicle info) {
        info.setVehicleId(UlidCreator.getMonotonicUlid().toString());
        int rows = emergencyVehicleMapper.insert(info);
        return AjaxResult.success(rows);
    }

    @Override
    @Log(title = "应急车辆", businessType = BusinessType.UPDATE)
    public AjaxResult update(EmergencyVehicle info) {
        EmergencyVehicle vehicle = emergencyVehicleMapper.selectById(info.getVehicleId());
        if (vehicle == null) {
            return AjaxResult.error("数据不存在");
        }
        int rows = emergencyVehicleMapper.update(info);
        return AjaxResult.success(rows);
    }

    @Override
    @Log(title = "应急车辆", businessType = BusinessType.DELETE)
    public AjaxResult delete(EmergencyVehicle info) {
        int rows = emergencyVehicleMapper.deleteById(info.getVehicleId());
        return AjaxResult.success(rows);
    }

    @Override
    public void export(EmergencyVehicleReq req, HttpServletResponse response) {
        req = initEmergencyVehicleReq(req);
        if (req == null) {
            return;
        }
        List<EmergencyVehicle> list = emergencyVehicleMapper.selectList(req);
        try (XSSFWorkbook workbook = ExcelUtils.getSheetAt("应急车辆导入模板.xlsx")) {
            if (workbook == null) {
                return;
            }
            ...
            if (list != null && !list.isEmpty()) {
                ...
                for (EmergencyVehicle item : list) {
                    ...
                    CellUtils.setStringVal(row, cellIndex++, item.getPlateNo(), style);
                    CellUtils.setStringVal(row, cellIndex++, item.getVehicleType(), style);
                    CellUtils.setStringVal(row, cellIndex++, item.getDriverName(), style);
                    CellUtils.setStringVal(row, cellIndex++, item.getDriverPhone(), style);
                    CellUtils.setStringVal(row, cellIndex++, item.getParkPlace(), style);
                    String statusText = item.getVehicleStatus() == null ? "" : (item.getVehicleStatus() == 1 ? "不可用" : "可用");
            ...
            response.setHeader("Content-Disposition", "attachment;filename*=UTF-8''" + URLEncoder.encode("应急车辆数据.xlsx", StandardCharsets.UTF_8));
            try (OutputStream outputStream = response.getOutputStream()) {
                workbook.write(outputStream);
            }
        } catch (Exception e) {
            log.error("导出应急车辆失败", e);
        }
    }
    ...
    public void importTemplate(HttpServletResponse response) {
        try (XSSFWorkbook workbook = ExcelUtils.getSheetAt("应急车辆导入模板.xlsx")) {
            ...
            response.setContentType(MimeTypeUtils.EXCEL_XLSX);
            response.setHeader("Content-Disposition", "attachment;filename*=UTF-8''" + URLEncoder.encode("应急车辆导入模板.xlsx", StandardCharsets.UTF_8));
            try (OutputStream outputStream = response.getOutputStream()) {
                workbook.write(outputStream);
            }
        } catch (Exception e) {
            log.error("导出应急车辆导入模板失败", e);
        }
    }
    ...
    public AjaxResult importExcel(MultipartFile file) {
        ...
        List<EmergencyVehicle> infos = new ArrayList<>();
        try (InputStream in = file.getInputStream(); Workbook wb = new XSSFWorkbook(in)) {
            ...
            for (int i = 3; i <= sheet.getLastRowNum(); i++) {
                ...
                String plateNo = CellUtils.getCellStringVal(row, index++);
                if (StringUtils.isBlank(plateNo)) {
                    continue;
                }
                String vehicleType = CellUtils.getCellStringVal(row, index++);
                String driverName = CellUtils.getCellStringVal(row, index++);
                String driverPhone = CellUtils.getCellStringVal(row, index++);
                String parkPlace = CellUtils.getCellStringVal(row, index++);
                String statusText = CellUtils.getCellStringVal(row, index++);
                ...
                if (StringUtils.isBlank(driverPhone)) {
                    errs.add("第" + rowNo + "行驾驶员电话为空");
                    continue;
                }

                EmergencyVehicle info = new EmergencyVehicle();
                info.setVehicleId(UlidCreator.getMonotonicUlid().toString());
                info.setEntCode(entCode);
                info.setPlateNo(plateNo);
                info.setVehicleType(vehicleType);
                info.setDriverName(driverName);
                info.setDriverPhone(driverPhone);
                info.setParkPlace(parkPlace);
                info.setVehicleStatus(parseVehicleStatusForImport(statusText));
        ...
        if (!infos.isEmpty()) {
            try {
                emergencyVehicleMapper.batchInsert(infos);
            } catch (Exception e) {
    ...
    private Integer parseVehicleStatusForImport(String vehicleStatusText) {
        if (StringUtils.isBlank(vehicleStatusText)) {
            return null;
        }
        String text = vehicleStatusText.trim();
        if ("可用".equals(text)) {
            return 0;
        }
        if ("不可用".equals(text) || "维修中".equals(text)) {
            return 1;
        }
        try {
    ...
    private Integer parseVehicleStatus(String vehicleStatus) {
        if (StringUtils.isBlank(vehicleStatus)) {
            return null;
        }
        try {
            return Integer.valueOf(vehicleStatus.trim());
        } catch (NumberFormatException e) {
            return null;
        }
    }
